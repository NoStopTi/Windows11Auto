<#
.SYNOPSIS
    Downloads and installs MySQL Workbench silently via winget.

.DESCRIPTION
    Ensures the Windows Package Manager (winget) is available on the machine,
    then uses it to download and install the latest MySQL Workbench package
    (published by Oracle) without any user interaction, prompt, or dialog.
    Only status messages are written to the console.
...
.EXAMPLE
    .\MySQLWorkbench.ps1
    Installs MySQL Workbench silently using the default package id.
...
[CmdletBinding()]
param(
    [string]$PackageId = "Oracle.MySQLWorkbench"
)
...
function Test-MySQLWorkbenchInstalled {
    # MySQL Workbench does not register a service, so the installed-marker
    # is its uninstall registry entry (checked in both the 64-bit and the
    # WOW6432Node 32-bit hive, since app entries can land in either one).
    $uninstallKeys = @(
    ...
    return [bool](
        ...
        Where-Object { $_.DisplayName -like "MySQL Workbench*" }
    )
}

try {
    # 1. Skip if already installed
    Write-Step "Checking if MySQL Workbench is already installed..."
    if (Test-MySQLWorkbenchInstalled) {
        Write-Host "MySQL Workbench is already installed." -ForegroundColor Yellow
    ...
    Write-Step "Installing MySQL Workbench ($PackageId)..."
    $installArgs = @(
        "install",
        "--id", $PackageId,
        "-e",
    ...
    if (Test-MySQLWorkbenchInstalled) {
        Write-Host "Success! MySQL Workbench was installed." -ForegroundColor Green
    }
    else {
