...
echo.
call :Color 78 "  KMS & KMS 2038 & Digital & Online Activation Suite %ver% - mephistooo2 - www.TNCTR.com" &echo:
echo.
...
echo.
call :Color 9 "     [2] ACTIVATION START FOR WINDOWS 10 (Digital & KMS 2038 Activation Method)" &echo:
echo.
call :Color 2 "     [3] ACTIVATION START FOR WINDOWS 10 & OFFICE (Online Activation Method)" &echo:
Echo.
call :Color 7 "     [4] WINDOWS & OFFICE ACTIVATION STATUS CHECK" &echo:
echo.
...
echo ============================================================================================
Echo.
echo choice /C:123456 /N /M "YOUR Choice :"

choice /T 5 /C 123456 /CS /D 3
...
echo.
echo choice /C:WOAM /N /M "YOUR CHOICE : "
choice /T 5 /C WOAM /CS /D A
...
set "_mO21a=Office 2021 C2R Retail is activated."
set "_mO19a=Office 2019 C2R Retail is activated."
set "_mO16a=Office 2016 C2R Retail is activated."
set "_mO15a=Office 2013 C2R Retail is activated."
set "_mO21c=Office 2021 C2R Retail could not be converted to Volume."
set "_mO19c=Office 2019 C2R Retail could not be converted to Volume."
set "_mO16c=Office 2016 C2R Retail could not be converted to Volume."
set "_mO15c=Office 2013 C2R Retail could not be converted to Volume."
set "_mO14c=Office 2010 C2R Retail is not supported by KMS Suite."
set "_mO14m=Office 2010 MSI Retail is not supported by KMS Suite."
set "_mO15m=Office 2013 MSI Retail is not supported by KMS Suite."
set "_mO16m=Office 2016 MSI Retail is not supported by KMS Suite."
set "_mOuwp=Office 365/2016 UWP is not supported by KMS Suite."
...
if %Debug% EQU 0 (
  ...
  echo This window will close automatically when the process is finished.) else (echo Please wait...)
  echo.
  echo "!_log!KMSInject.log"
  )
  @echo on
  @prompt $G
  @call :Begin >"!_log!_tmp.log" 2>&1 &cmd /u /c type "!_log!_tmp.log">"!_log!KMSInject.log"&del "!_log!_tmp.log"
)
@exit /b
::===============================================================================================================
:Begin
...
echo Press any key to test1_AQUI...
echo Pause >nul 2>&1
CLS
echo call %~dp0KMS_Suite.cmd -suite
shutdown -r -f -t 00
)
...
:IntConnected
echo.
echo    Searching for Online KMS Server, please wait...
set "srvpri="
set "srvsec="
...
echo.
if %External% EQU 0 if not exist "!_work!\bin\%xOS%.dll" goto :DLL
call :StopService sppsvc
...
if %WinPerm% EQU 1 (
for /f "tokens=2 delims==" %%x in ('"wmic path %spp% where (ApplicationID='%_wApp%' and LicenseStatus='1' %adoff%) get Name /VALUE"') do echo.
echo. %NameOS% %SP%%xOS% is Permanently Activated.
exit /b
)
call :insKey
exit /b
...
::===============================================================================================================
:win7off
if %loc_off21% EQU 0 if %loc_off19% EQU 0 if %loc_off16% EQU 0 if %loc_off15% EQU 0 if %loc_off14% EQU 0 (echo.&echo. &exit /b)
set Off1ce=1
set _sC2R=win7off
...
%EchoRed% Unsuccesful: 0x!=ExitCode!
set S_OK=0
exit /b
)
...
for /f "tokens=2 delims==" %%x in ('"wmic path %spp% where ID='%app%' get Name /VALUE"') do echo. %%x
...
echo.
echo Microsoft 365 product activated with a subscription.
)
if %sub_proj% EQU 1 (
  for %%a in (%_PrjSKU%) do set _%%a=0
echo.
echo Microsoft Project product activated with a subscription.
)
if %sub_vis% EQU 1 (
  for %%a in (%_VisSKU%) do set _%%a=0
echo.
echo Microsoft Visio product activated with a subscription.
)
...
If %Auto% EQU 1 (
taskkill /f /im DisableX.exe >nul
...
del /f /q "%systemroot%\System32\KMS.dll" >nul 2>&1
echo Press any key to continue...
echo Press any key to test_AQUI2...
echo Pause >nul 2>&1
CLS
echo call %~dp0KMS_Suite.cmd -suite
shutdown -r -f -t 00
...
:Exit
cls
taskkill /f /im DisableX.exe >nul 2>&1
...
taskkill /f /fi "WindowTitle eq  Administrator:  "*"" /t >nul 2>&1 &exit
::===============================================================================================================
:Color_Pre
